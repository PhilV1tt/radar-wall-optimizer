...
import matplotlib
matplotlib.use('Agg')  # Backend sans affichage pour le serveur
...
# ==============================================================================
# Fonction de fitness (multi-angles)
# ==============================================================================
...
def run_baseline():
    ...
    # Fitness baseline = somme sur tous les angles
    flat_fitness = _evaluate_wall(flat_params)

    print(f"  Énergie rétrodiffusée (mur plat, {len(INCIDENCE_ANGLES)} angles) : {flat_fitness:.6f}")
...
def run_cma_optimization(ga_best_genome=None):
    print("\n" + "=" * 70)
    print(f"PHASE 2b : CMA-ES ({N_WORKERS} workers, warm start depuis GA)")
    print("=" * 70)
...
def run_rl_optimization():
    print("\n" + "=" * 70)
    print(f"PHASE 3 : Reinforcement Learning ({len(INCIDENCE_ANGLES)} angles)")
    print("=" * 70)
...
def run_comparison(flat_fitness, ga_best, ga_history,
    ...
    print(f"\n  Angles d'incidence     : {INCIDENCE_ANGLES}")
    print(f"  Total simulations FDTD : {sc}")
