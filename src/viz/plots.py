...
def plot_field_snapshot(Ez: np.ndarray, pec_mask: np.ndarray,
    ...
    plt.colorbar(im, ax=ax, shrink=0.8, label='Ez (V/m)')
...
def plot_field_snapshots_grid(snapshots: List[Tuple[np.ndarray, np.ndarray, str]],
                               save_path: Optional[str] = None):
    """Trace une grille de snapshots (évolution temporelle)."""
    n = len(snapshots)
    cols = min(3, n)
    rows = (n + cols - 1) // cols
    ...
    for idx, (Ez, pec, title) in enumerate(snapshots):
        ax = axes[idx]
        norm = TwoSlopeNorm(vmin=-vmax, vcenter=0, vmax=vmax)
        ax.imshow(Ez.T, origin='lower', cmap='RdBu_r', norm=norm,
                  aspect='equal', interpolation='bilinear')
...
def plot_project_summary(sim_config, ga_history, rl_history,
    ...
    # Panel 4 : Texte résumé
    ax4 = fig.add_subplot(gs[1, :])
    ax4.axis('off')

    summary_text = (
        f"RÉSUMÉ DU PROJET : Optimisation de mur anti-radar par ML\n"
        f"Simulation : FDTD 2D TMz ({sim_config.nx}x{sim_config.ny}, "
        f"f={sim_config.freq/1e9:.0f} GHz, lambda={sim_config.wavelength*100:.1f} cm)\n"
        f"Résolution : {sim_config.ppw} points/lambda, "
        f"dx={sim_config.dx*1e3:.2f} mm, Sc={sim_config.courant}\n\n"
        f"Mur plat (baseline) : RCS = {flat_fitness:.6f}\n"
        f"GA optimal          : RCS = {ga_history['best_fitness'][-1]:.6f}  "
        f"(réduction: {(1-ga_history['best_fitness'][-1]/flat_fitness)*100:.1f}%)\n"
        f"RL optimal          : RCS = {rl_history['best_fitness'][-1]:.6f}  "
