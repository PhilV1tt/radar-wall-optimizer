...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\textit{Radar Wall Optimizer --- Rapport technique}}
\fancyhead[R]{\small\textit{FDTD 2D + Algorithmes d'optimisation}}
\fancyfoot[C]{\thepage}
...
\title{
    \vspace{-1cm}
    {\large\textsc{Radar Wall Optimizer} \\[2pt]
     \textsc{Documentation technique du logiciel de simulation}}\\[1.5cm]
...
\begin{abstract}
\noindent
Ce document constitue le rapport technique du logiciel \textit{Radar Wall Optimizer},
un outil de simulation et d'optimisation pour la réduction de la Section Efficace
Radar (SER) d'un mur métallique parfaitement conducteur (PEC), dans le contexte
de la bande X radar (\SI{10}{\giga\hertz}). Le logiciel intègre un simulateur
électromagnétique complet par la méthode des différences finies dans le domaine
temporel (FDTD 2D) en polarisation TMz, avec couche d'absorption CPML et injection
de source par technique champ-total/champ-diffusé (TFSF). L'espace de conception
est paramétré par $N_s = 14$--$16$ points de contrôle décrivant le profil de
surface de la paroi. Trois méthodes d'optimisation sont implémentées et comparées~:
un algorithme génétique (GA) avec croisement SBX et mutation polynomiale,
l'algorithme CMA-ES initialisé depuis le meilleur individu GA, et l'algorithme
REINFORCE de gradient de politique. La fonction objectif mesure l'énergie
rétrodiffusée calculée dans la zone de champ diffusé. Des réductions de SER
supérieures à 90\,\% sont obtenues en incidence normale~; le profil optimal
converge systématiquement vers une géométrie en dents de scie à pas de l'ordre
de $\lambda/4$, dont la physique est discutée.

Ce rapport détaille les fondements théoriques, les choix d'implémentation
et les résultats obtenus, en vue d'accompagner un projet de physique portant
sur la furtivité radar.
...
Le logiciel \textit{Radar Wall Optimizer} s'inscrit dans la deuxième approche~: il
permet de déterminer numériquement la \textit{géométrie optimale} d'un mur PEC plan
pour minimiser sa SER monostatique à \SI{10}{\giga\hertz} (bande X radar). Il a été
conçu comme outil de simulation et d'exploration pour accompagner un projet de
physique portant sur la furtivité radar et l'optimisation électromagnétique.
...
Les contributions principales du logiciel sont les suivantes~:
\begin{itemize}
    ...
    \item Implémentation et comparaison de trois algorithmes d'optimisation
          (GA, CMA-ES, REINFORCE) sur un problème de minimisation de SER~;
...
Le logiciel \textit{Radar Wall Optimizer} constitue un outil numérique complet
pour l'optimisation de la géométrie d'un mur PEC en vue de minimiser sa Section
Efficace Radar à \SI{10}{\giga\hertz}. Le simulateur FDTD 2D TMz intégré, avec
source TFSF, couche absorbante CPML et transformation champ proche--champ lointain,
a été validé par comparaison avec la référence analytique d'Optique Physique.

Trois algorithmes d'optimisation sont implémentés et comparés~: l'algorithme
...
L'outil est pensé pour accompagner un projet de physique sur la furtivité radar~:
il permet de visualiser les phénomènes de diffusion, d'explorer l'espace de design
des géométries et de comprendre l'impact de chaque paramètre sur la SER.
